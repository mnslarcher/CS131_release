...
class PCA(object):
    ...
    def transform(self, X, n_components):
        ...
        X_proj = X.copy()
        X_proj -= self.mean
        X_proj = np.dot(X_proj, self.W_pca[:, :n_components])
        # END YOUR CODE
...
class LDA(object):
    ...
    def _within_class_scatter(self, X, y):
        ...
        for i in np.unique(y):
            ...
            mui = Xi.mean(axis=0)
            Xi_centered = Xi - mui
            scatter_within += np.dot(Xi_centered.T, Xi_centered)
            # END YOUR CODE
    ...
    def _between_class_scatter(self, X, y):
        ...
        for i in np.unique(y):
            # YOUR CODE HERE
            Ni = (y == i).sum()
            mui_centered = (X[y == i].mean(axis=0) - mu)[:, None]
            scatter_between += Ni * np.dot(mui_centered, mui_centered.T)
            # END YOUR CODE
